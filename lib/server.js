...
app.get('/proxy', function proxyFeed(req, res) {
  ...
  // monkey patch
  let feedUrl = decodeURIComponent(req.query.url);
  feedUrl = feedUrl.replace(/^https:\/\/feeds\.earhustlesq\.com/, 'http://feeds.earhustlesq.com');

  let parts = url.parse(feedUrl);
  let requester = (parts.protocol === 'http:' ? http : https);
  let proxyReq = requester.request(parts.href, (proxyRes) => {
