...
exports.buildIndex = (isDist) => {
  ...
  cachedMarkup = (isDist && cachedMarkup) ? cachedMarkup : exports.getLoadingMarkup(isDist);
...
exports.getLoadingMarkup = (isDist) => {
  if (isDist) {
    return inlineLoadingPlayer("../dist/app/shared/player/player.component.css", "../dist/app/shared/player/player.component.html");
  } else {
    return inlineLoadingPlayer("../src/app/shared/player/player.component.css", "../src/app/shared/player/player.component.html");
  }
}
...
function inlineLoadingPlayer(cssPath, htmlPath) {
  ...
  return inlineCss(`<style>${cssContent}</style>
    ...
    scrub($, $('.loadingRender').children());
    return `<style>${mediaQueries}</style>${$('.loadingRender').html()}`;
  });
}

function scrub($, elems) {
  elems.each(function () {
    scrubElem($, $(this));
  });
}

function scrubElem($, elem) {
